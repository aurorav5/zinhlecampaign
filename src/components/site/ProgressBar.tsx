...
export function ProgressBar({ raised, target, currency = "ZAR" }: Props) {
  const pct = Math.max(0, Math.min(100, target > 0 ? (raised / target) * 100 : 0));
  return (
    <div>
      <div className="flex items-baseline justify-between text-sm text-[color:var(--cream)]/75">
        ...
        <span>of {fmt(target, currency)}</span>
      </div>
      <div
      ...
      <div className="mt-2 text-xs text-[color:var(--cream)]/55">{pct.toFixed(1)}% funded</div>
    </div>
  );
}
